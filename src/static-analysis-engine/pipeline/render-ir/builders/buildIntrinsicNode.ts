...
function summarizeClassAttribute(
  ...
  for (const property of attributes.properties) {
    ...
    const sourceAnchor = toSourceAnchor(expression, context.parsedSourceFile, context.filePath);
    const value = summarizeBoundClassNameExpression(expression, context);

    return {
      sourceAnchor,
      value,
      classes: toAbstractClassSet(value, sourceAnchor),
      sourceText: expression.getText(context.parsedSourceFile),
    };
  }
